            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <div class="card card-custom gutter-b example example-compact">
                        <!--begin::Form-->
                        <form action="{{ url('backend/permissions/'.$permission->id) }}" method="POST"  id="frmAdd">
                            @method('PUT')
                            <div class="card-body">
                                <div class="form-group row">
                                    <label  class="col-lg-3 col-form-label" for="Permission Name">Permission Name<span class="required">*</span></label>
                                    <div class="col-lg-6">
                                        <input id="permission_name" type="text" class="form-control required" name="permission_name" value="{{ $permission->name }}" placeholder="Permission Name" >
                                    </div>
                                </div>
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-6">
                                        <select class="form-control city custom-select required" name="model_name"
                                            ...
                                            @foreach($models as $model)
                                                <option value="{{ $model }}" {{ old('model_name', $permission->model_name) == $model ? 'selected' : '' }}>{{ $model }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error('model_name')
